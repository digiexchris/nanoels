...
volatile std::atomic< long> spindlePosDelta; // Unprocessed encoder ticks.
...
volatile std::atomic<int> pulse1Delta; // Outstanding pulses generated by pulse generator on terminal A1.
volatile std::atomic<int> pulse2Delta; // Outstanding pulses generated by pulse generator on terminal A2.
...
volatile std::atomic<unsigned long> pulse1HighMicros;
volatile std::atomic<unsigned long>  pulse2HighMicros;
...
void IRAM_ATTR pulse1Enc() {
  ...
  } else if (now > pulse1HighMicros.load() + PULSE_MIN_WIDTH_US) {
    //pulse1Delta.
    pulse1Delta.fetch_add((DREAD(A13) ? -1 : 1) * (PULSE_1_INVERT ? -1 : 1));
  }
}
...
void setup() {
  ...
  pulse1Delta.store(0);
  pulse2Delta.store(0);
  pulse1HighMicros.store(0);
  pulse2HighMicros.store(0);
  spindlePosDelta.store(0);
